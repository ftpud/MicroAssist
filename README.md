...
In `TIMEZONE` local time, the server checks refresh eligibility at 07:00 and every two hours from 09:00 through 21:00. It calls Codex only when `MEMORY.md`, `TASKS.md`, `RECURRING.md`, or the local calendar day changed; unchanged checks are logged and skipped. A successful non-deterministic prompt rebuilds `ACTUAL.md` in the same model turn. Simple relative reminders and immediate notification commands are handled directly without consuming a Codex turn. At 02:30 it copies `workspace/` to `backups/` and retains the latest 14 snapshots.

Recurring content is user-controlled. A prompt such as “Каждое утро в 10 присылай мне новый мотивационный пост на основе задач” creates an enabled cron entry in `RECURRING.md`. At creation time Codex selects the minimal future context files (for example `TASKS.md` and `ACTUAL.md`); each occurrence receives only that profile plus the card state. The server notices schedule changes without a restart, asks Codex to generate fresh content when the event fires, creates a card, and delivers its notification. The app's “Таймеры” tab lists recurring events and one-time reminders; swipe either one left to delete it.
